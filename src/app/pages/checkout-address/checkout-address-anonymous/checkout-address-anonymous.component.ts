...
export class CheckoutAddressAnonymousComponent implements OnChanges {
  ...
  private scrollToFirstInvalidField() {
    if (this.form.controls.invoiceAddress?.invalid) {
      FormsService.focusFirstInvalidFieldRecursive(this.form.controls.invoiceAddress as FormGroup, this.invoiceFormId);
      return;
    }
    if (this.form.controls.shippingAddress?.invalid) {
      FormsService.focusFirstInvalidFieldRecursive(
